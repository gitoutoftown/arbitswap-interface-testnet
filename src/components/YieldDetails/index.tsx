...
const YieldDetails: React.FC<YieldDetailsProps> = ({
  ...
  const perDay: number = Number((1000 * roiPerDay) / aswapPrice)
  const perWeek: number = Number((1000 * roiPerWeek) / aswapPrice)
  const perMonth: number = Number((1000 * roiPerMonth) / aswapPrice)
  const perYear: number = Number((1000 * roiPerYear) / aswapPrice)
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2 flex flex-col">
        <div className="flex flex-row flex-nowrap gap-1 font-bold">
          ...
          <div className="flex flex-row py-1 px-2 w-full">
            <div className="flex items-center justify-between uppercase">{i18n._(t`ASWAP per $1000`)}</div>
          </div>
        </div>
